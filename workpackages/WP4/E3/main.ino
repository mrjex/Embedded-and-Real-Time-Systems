...
#define INTERRUPT_INTERVAL 0.1 // interrupt ever 0.1 seconds
#define NUM_LEDS 12 // define the maximum number of strip leds
#define MAX_TEMP 125 // define the maximum possible temperature
	
#define TEMP_SENSOR A5 // define the pin for the temp sensor 
#define LED_PIN 6 // define the pin for the led sensor
#define RED_LED 2 // define the pin for the red light sensor


// define the neopixel struct
// 12 leds
// pin number 6
// set the expected frequency value
...
void setup()
{
 // start the serial monitor
  ...
  // set the temperature sensor mode to input
  pinMode(TEMP_SENSOR, INPUT);
  
  // start the led strip
  strip.begin();

  // initialize timer 1 with an interval of .1 seconds
  Timer1.initialize(INTERRUPT_INTERVAL * 1E6); //Initialize timer with 1 second period
  // attack the interrupt callback
...
void callback() {
  ...
  // define a boolean allOn initially set to true
  bool allOn = true;
  
  // for each LED
  for(byte i=0; i<NUM_LEDS; i++) {
    
    // calculate the threshold for that index
    int threshold = (MAX_TEMP / NUM_LEDS) * i;
    
    // if the current temperature exceeds the threshold
    if(celsius >= threshold) {
      // turn on that specific light
        strip.setPixelColor(i, 255, 0, 255);   
    }
    else {
      // otherwise turn off that light
        strip.setPixelColor(i, 0, 0, 0);
      // set allOn to false
      	allOn = false;
    }
  }
  // show the light strip
  strip.show();
  // toggle the red led based on if all strip leds are on
  digitalWrite(RED_LED, allOn ? HIGH : LOW);
