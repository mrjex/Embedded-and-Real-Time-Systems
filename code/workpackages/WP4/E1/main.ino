...
#define MAX_LEDS 5 // define max number of leds
#define INTERRUPT_INTERVAL 0.1 // interrupt every 0.1 seconds
#define TEMP_SENSOR A5 // define the temp sensor at A5

struct Light {
  	byte pin;
	int minimum;
  	int maximum;
}; // define a struct Light
// holds the pin, the minimum threshold and the maximum threshold

// define the minimum and maximum range for leds
// maximum is not used in this implementation
// but if its intended to turn off the light at a specific
// maximum value, it is made possible.
const struct Light LEDS[MAX_LEDS] = {
  {.pin = 2, .minimum = 0,  .maximum = 10},
  {.pin = 3, .minimum = 11, .maximum = 20},
  {.pin = 4, .minimum = 21, .maximum = 30},
  {.pin = 5, .minimum = 31, .maximum = 40},
  {.pin = 6, .minimum = 41, .maximum = 50},
};
...
void setup()
{
  ...
  // initialize timer1
  // with interval 0.1 seconds
  Timer1.initialize(INTERRUPT_INTERVAL * 1E6); //Initialize timer with 1 second period
  
  // attack the callback interrupt
  Timer1.attachInterrupt(callback);
  
  // set all led pin modes to output
  for(int i = 0; i < MAX_LEDS; i++) {
  		pinMode(LEDS[i].pin, OUTPUT); // set the pin mode to output
  }
  
  // set the temperature sensor pin mode to input
...
void callback() {
  ...
  float signal = analogRead(TEMP_SENSOR);
  // convert the analog signal to volts
  float voltage = signal * 5; // 5 volts
  voltage /= 1024;
  
  // convert the volts to celsius
  // subtract 500mv to account for negative numbers
  ...
  // for each led
  for(int i = 0; i < MAX_LEDS; i++) {
    // if the temp has exceeded that leds minimum threshold
    if(celsius >= LEDS[i].minimum) {
      // turn on the led
